...
const CharacterItem: React.FC<CharacterItemProps> = ({ character }) => {
  return (
    <div className="character-item p-4 border rounded md:flex md:items-start">
      <img
        src={`https://picsum.photos/200/300?random=${character.id}`}
        alt={character.name}
        className="character-image w-full h-48 object-cover mb-4 md:h- md:mb-0 md:mr-4"
      />
      <div className="character-text w-full md:w-2/3">
